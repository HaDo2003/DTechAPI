...
namespace DTech.API.Controllers.Admin
{
    ...
    ) : ControllerBase
    {
        ...
        [HttpPost("create")]
        public async Task<IActionResult> CreateCoupon([FromBody] CouponDetailDto model)
        {
        ...
        [HttpPut("update/{id}")]
        public async Task<IActionResult> UpdateCoupon(int id, [FromBody] CouponDetailDto model)
        {
            var (userId, unauthorized) = ControllerHelper.HandleUnauthorized(User, this);
            if (unauthorized != null) return unauthorized;
