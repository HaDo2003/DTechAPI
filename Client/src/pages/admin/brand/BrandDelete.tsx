...
const BrandDelete: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { token } = useAuth();
    const [data, setData] = useState<BrandForm | null>(null);
        ...
        if (id) {
            (async () => {
                ...
                if (res.success && res.data) {
                    ...
                    setData({
                        ...brand,
                        statusName: brand.status === 1 ? "Available" : "Unavailable",
                    });
                }
            })();
        }
    ...
    const handleDelete = async () => {
        setLoading(true);
        const res = await adminService.deleteData(`/api/brand/delete`, id ?? "", token ?? "");
        if (res.success) {
            setLoading(false);
